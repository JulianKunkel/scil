...
error tolerance) or if lossy compression suffices the users needs. \par
In case of lossless compression the parameter restricting compression speed is
the only one consulted. If none is given, SCIL will default to best compression
rates regardless of compression speed. \par
If the user provided arguments describing error tolerance, SCIL considers lossy
algorithms, optinally with a subsequent lossless compression. \par
In either case the platform specific configuration of SCIL [see subsec ...] and
current data characteristics are of interest. Analizing the data for its
characteristics is an optional feature as it is very time consuming. Yet it
enables further customization of the compression by the user.

\bigskip

\subsection{Writing Data Header}
...
Running the program 'scil-config' overwrites
the default configuration file for scil, adapting SCIL to the platform it will
be used on.
