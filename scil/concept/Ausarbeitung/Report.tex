...
data, abstol packs the buffer tightly as described in \cref{}
The bit count is
...
Encoding of numbers can then be done by using the following function:
...
The sigbits compression algorithm uses the provided relative error tolerance.
  ...
  by projecting uncompressed values to their
nearest representating value, based on the user provided relative error
tolerance. After determining the minimum and maximum values of the data, abstol
calculates how many bits are needed to represent all numbers in the
minimum-maximum-range, given the absolute error tolerance. The bit count is
calculated as follows:
